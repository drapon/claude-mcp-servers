 ...
 * @param userAgent 使用するUserAgent
 */
async function checkMayAutonomouslyFetchUrl(
  url: string,
  userAgent: string,
): Promise<void> {
  ...
  async (args, extra) => {
    try {
      ...
      if (!url) {
        return {
          content: [
            { type: 'text', text: `Error: ${ERROR_MESSAGES.URL_REQUIRED}` },
          ],
          isError: true,
        }
      }
      ...
      // UserAgentの設定
      const userAgent =
        globalOptions.customUserAgent || DEFAULT_USER_AGENT_AUTONOMOUS
...
server.prompt(
  ...
  async (args, extra) => {
    try {
      ...
      // UserAgentの設定
      const userAgent =
        globalOptions.customUserAgent || DEFAULT_USER_AGENT_MANUAL
