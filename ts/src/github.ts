 ...
 * リスト操作系のデフォルト値を設定する
 */
function applyIssueListDefaults(
  options: Partial<interfaces.ListIssuesArguments>,
) {
  return {
 ...
 * PRリストのデフォルト値を設定する
 */
function applyPullListDefaults(
  options: Partial<interfaces.ListPullRequestsArguments>,
) {
  return {
    state: options.state || 'open',
    sort: options.sort || 'created',
...
function applyMergeDefaults(
  options: Partial<interfaces.MergePullRequestArguments>,
) {
  return {
