...
const customTemplatePath = path.join(
  projectRoot,
  "claude_desktop_config.custom.json"
);
...
async function setupConfig() {
  try {
    console.log(
      "Starting interactive setup for Claude Desktop MCP configuration..."
    );
    ...
    try {
      ...
      console.warn(
        "Warning: .env file not found or cannot be read. Will create one."
      );
    }
    ...
    serverSelections.enabledMcps.forEach((server) => {
      updatedTemplate.mcpServers[server] = JSON.parse(
        JSON.stringify(templateJson.mcpServers[server])
      );
          ...
          if (
            ...
            if (match && match[1]) {
              ...
              if (varName !== "PROJECT_ROOT") {
                // Skip PROJECT_ROOT as it's handled dynamically
    ...
    if (envQuestions.length > 0) {
      console.log(
        "Some required environment variables are missing. Please provide them:"
      );
    ...
    execSync(
      `bun run scripts/generate-config.mjs --template ${customTemplatePath}`,
      {
        stdio: "inherit",
      }
    );
